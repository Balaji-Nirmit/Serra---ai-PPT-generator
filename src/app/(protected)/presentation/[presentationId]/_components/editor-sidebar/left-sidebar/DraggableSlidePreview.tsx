...
const DraggableSlidePreview =({slide,index,moveSlide}:Props)=>{
    ...
    return (
        <>
        <div ref={ref} className={cn('relative cursor-pointer group',
            index===currentSlide?'before:bg-blue-500':'before:bg-transparent',
            isDragging?'opacity-50':'opacity-100',
        )}
        onClick={()=>setCurrentSlide(index)}
        >
            {/* <div className="pl-2 mb-4 relative border-red-600 border-2 min-h-[100px]"> */}
                <ScaledPreview slide={slide} isActive={index === currentSlide} index={index}/>
            {/* </div> */}
        </div>
        </>
    )
