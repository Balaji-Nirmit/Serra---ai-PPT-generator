...
export const DraggableLayoutItem = ({
    component, icon, layoutType, name, type
}: Layout) => {
    ...
    return <div ref={drag as unknown as React.LegacyRef<HTMLDivElement>}
        style={{
    ...
    >
        <LayoutPreviewItem name={name} Icon={icon} type={type} component={component} />
    </div>
}
            ...
            <ScrollArea className="h-[400px]" style={{
                ...
                <div className="p-4">
                    {layouts.map((group) => (<div key={group.name} className="mb-6">
                        <h3 className="text-sm font-medium my-4 text-muted-foreground">{group.name}</h3>
