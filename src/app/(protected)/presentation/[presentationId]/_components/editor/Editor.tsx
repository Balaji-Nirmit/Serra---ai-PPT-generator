    ...
    return (
        <>
            <div ref={dropRef as unknown as React.RefObject<HTMLDivElement>}
                className={cn('h-4 my-2 rounded-md transition-all duration-200', isOver && canDrop ? 'border-green-500 bg-green-100' : 'border-gray-300', canDrop ? 'border-blue-300' : '')}>
                {isOver && canDrop && (
                    <div className="h-full flex items-center justify-center text-green">DROP LAYOUT HERE</div>
                )}
            </div>
        </>)
}
export const DraggableSlide: React.FC<DraggableSlideProps> = ({ slide, index, moveSlide, handleDelete, isEditable }) => {
    ...
    return <div ref={ref}
        className={cn('w-full rounded-lg shadow-lg relative p-0 min-h-[400px]', 'shadow-xl transition-shadow duration-300', 'flex flex-col', index === currentSlide ? 'ring-2 ring-blue-500 ring-offset-2' : '', slide.className, isDragging ? 'opacity-50' : 'opacity-100')}
        style={{
    ...
    >
        <div className="h-full w-full flex-grow overflow-hidden">
            <MasterRecursiveComponent content={slide.content} isPreview={false} slideId={slide.id} isEditable={isEditable} onContentChange={handleContentChange} />
            {isEditable && (
                <Popover>
...
const Editor = ({
    ...
    const handleDrop = (
        ...
        if (item.type === 'layout') {
            addSlideAtIndex({ ...item.component, id: v4(), slideOrder: dropIndex }, dropIndex)
    ...
    // down is for saving the changes when edit is done through throttling
    ...
    useEffect(() => {
        // is we already have a timer cancel it and create new one
        if (autoSaveTimerRef.current) {
            clearTimeout(autoSaveTimerRef.current)
        }

        // inside the timer make save request  
        if (isEditable) {
    ...
    return (
        <>
            <div className="flex-1 flex flex-col h-full max-w-5xl mx-auto px-4 mb-20">
                {loading ?
