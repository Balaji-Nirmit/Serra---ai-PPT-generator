...
const CardList = ({
    ...
    const getDragOverStyles = (cardIndex: number): React.CSSProperties => {
                    ...
                    {outlines.map((item, index) => (
                        <React.Fragment key={item.id}>
                            <Card
                                ...
                                card={item}
                                isEditing={editingCard === item.id}
                                isSelected={selectedCard === item.id}
                                editText={editText}
                                onEditChange={onEditChange}
                                onEditBlur={() => onCardUpdate(item.id, editText)}
                                onEditKeyDown={(e) => {
                                    if (e.key === "Enter") {
                                        onCardUpdate(item.id, editText);
                                    }
                                }}
                                onCardClick={() => onCardSelect(item.id)}
                                onCardDoubleClick={() => onCardDoubleClick(item.id, item.title)}
                                onDeleteClick={() => onCardDelete(item.id)}
                                dragHandlers={{
