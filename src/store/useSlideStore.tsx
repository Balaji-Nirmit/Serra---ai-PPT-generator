import { ContentItem, Projects, Slide, Theme } from '@/lib/types'
...
export const useSlideStore = create(
    persist<SlideState>(
        (set, get) => ({
            ...
            project: null,
            setProject: (project: Projects) => set({ project }),
            ...
            removeSlide: (id: string) => {
                set((state) => ({
                    slides: state.slides.filter((item) => item.id !== id)
                }))
            },
            ...
            updateContentItem: (slideId, contentId, newContent) => {
                set((state) => {
                    const updateContentRecursively = (item: ContentItem): ContentItem => {
                        ...
                        if (Array.isArray(item.content) && item.content.every((s) => typeof s !== 'string')) {
                            return {
                        ...
                        if(slide.id===slideId){
                            const updateContentRecursively=(content:ContentItem):ContentItem=>{
                                if(content.id === parentId && Array.isArray(content.content)){
                                    ...
                                    return {
                                        ...content, content:updatedContent as unknown as string[]
                                    }
                                }
                                return content
                            }
        ...
        {
            name: 'slide-storage' // unique name
        }
    )
) 