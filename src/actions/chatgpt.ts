...
export const generateCreativePrompt = async (userPrompt: string) => {
    ...
    Ensure that the JSON is valid  and properly formatted. Do not include any other text or explanations outside the JSON structure.
    `
    ...
    try {
        ...
        return { status: 500, error: "Failed to generate creative prompt." };
    }
}
...
- Focus on accurately depicting the concept described, including specific objects, environment, mood, and context. Maintain relevance to the description provided.
    ...
    return results?.data?.results[0].urls.regular;
    // return 'https://static.vecteezy.com/system/resources/previews/002/280/269/non_2x/light-purple-background-with-random-forms-vector.jpg'
}
const replaceImagePlaceholders = async (layout: any): Promise<void> => {
    const imageComponents = findImageComponents(layout.content);
    // console.log('Found image components:', 
    //     imageComponents
    // );
    for (const component of imageComponents) {
        // console.log('Generating image for component:', 
        //     component.alt
        // );
...
export const geenrateLayoutsJson = async (outlineArray: string[]) => {
...
- Avoid using terms like "image of" or "picture of" and instead focus directly on content and meaning.
...
export const generateLayouts = async (projectId: string, theme: string) => {
    try {
        if (!projectId || !theme) {
            return { status: 400, error: "Project ID and theme are required to generate layouts." }
        }
        const project = await db.select().from(projectsTable).where(and(eq(projectsTable.id, projectId), eq(projectsTable.isDeleted, false)));
        if (!project || project.length === 0) {
            return { status: 404, error: "Project not found." }
        }
        if (!project[0].outlines || project[0].outlines.length === 0) {
            return { status: 400, error: "Project outlines are required to generate layouts." }
        }
        const layouts = await geenrateLayoutsJson(project[0].outlines);
        if (layouts.status !== 200) {
            return layouts
        }
        await db.update(projectsTable).set({ slides: layouts.data, themeName: theme }).where(eq(projectsTable.id, projectId));
