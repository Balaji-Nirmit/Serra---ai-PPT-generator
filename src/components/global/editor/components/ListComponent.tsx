...
const NumberedList: React.FC<ListProps> = ({
    items, onChange, className, isEditable = true
}) => {
    ...
    return (
        <>
            <ol className={cn(
                'list-decimal list-inside space-y-1', className
            )} style={{
...
export const BulletList: React.FC<ListProps> = ({ items, onChange, className, isEditable = true}) => {
    ...
    return <ul className={cn(
        'list-disc pl-5 space-y-1', className
    )} style={{
        color: currentTheme.fontColor
    }}>
        {items.map((item, index) => (
        ...
        {items.map((item, index) => (
            <li key={index} className="flex items-center space-x-2">
                ...
                <ListItem
                    item={item.replace(/^\[[ x]\] /,'')}
                    index={index}
                    onChange={(index,value)=>handleChange(
                        index,`${item.startsWith('[x] ')?'[x] ':'[ ] '}${value}` 
                    )}
                    onKeyDown={handleKeyDown}
                    isEditable={isEditable}
                    fontColor={currentTheme.fontColor}
